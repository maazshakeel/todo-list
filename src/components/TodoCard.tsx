...
export default function TodoCard({ children }: TodoCardProps) {
  const { isDeleteModelOpen, setIsDeletemodelOpen, deleteTodo } =
  ...
  return (
    <div className="group text-white bg-[#272b33] mt-4 h-[70px] w-full border-[1px] border-[#7a828e] rounded-md border-solid flex-shrink-0 shadow-inner overflow-visible pr-4 pl-[10px] pt-[10px] pb-2 flex flex-col gap-2 hover:cursor-grab">
      {" "}
      {/*//onClick={() => alert(children)}*/}
      {isDeleteModelOpen ? (
        <>
          <div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none hover:cursor-default">
            <div className="relative w-96 my-6 mx-auto max-w-3xl">
              ...
              >
                ...
                <div className="flex items-start justify-between pt-[2px]">
                  <h3 className="text-[18px] font-semibold text-[#f0f3f6]">
                    Delete todo?
                  </h3>
                </div>
      ...
      <div className="flex flex-wrap items-center justify-between  select-none">
        ...
        <AiOutlineDelete
          ...
          className="text-red-500 group-hover:block hidden hover:cursor-pointer"
        />
        {/* <p className="text-[#f0f3f6] text-[12px] group-hover:block hidden">Delete</p> */}
      </div>
      <span className="text-[#f0f3f6] text-[14px] hover:text-[#71b7ff] hover:cursor-pointer hover:underline w-fit">
        {children.todo}
      </span>
    </div>
  );
